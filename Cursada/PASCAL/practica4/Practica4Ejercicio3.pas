...
Procedure CodMaxUniVendidas(a:arbol; var max: integer; var codmax:integer);

begin
  
   if (a<>nil) then begin          
           CodMaxUniVendidas(a^.hi,max,codmax);
           maximo(a,max,codmax);
           CodMaxUniVendidas(a^.hd,max,codmax);
        end;
...
begin
    ...
    if (a = nil) then menores:= 0
                 ...
                 else if( valor < a^.dato.codigo) then menores:= menores (a^.hi,valor);
...
Function CalcularMonto(a:arbol;num1,num2: integer):real;
var monto:real;
begin
   monto:= 0;
   if (a= nil) then monto:= 0
            else  if ((a^.dato.codigo > num1) and (a^.dato.codigo < num2)) then begin
                    monto:= a^.dato.MontoTotal + CalcularMonto(a^.hi,num1,num2); CalcularMonto(a^.hd,num1,num2);
                  end
                          else if (a^.dato.codigo < num1) then monto:= CalcularMonto(a^.hi,num1,num2)
                                                          else if (a^.dato.codigo > num2) then monto:= CalcularMonto(a^.hd,num1,num2);
   CalcularMonto:= monto;
end;
...
begin
 ...
 If(Total > 0) then writeln('El monto total de los productos cuyo codigo se encuentra entre ' , num1 , ' y ' , num2, ' es ' , total:4:2)
...
var a: arbol; max, codmax: integer;
...
writeln ('----- Modulo C ----->');
writeln;
CodMaxUniVendidas(a,max,codmax);
